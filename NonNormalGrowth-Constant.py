...
mm = UnitSquareMesh(600, 300)  #Main grid
# mm = UnitSquareMesh(1200, 600) #Fine grid
nn = 600
mm.coordinates.dat.data[:,0] -= 0.5
mm.coordinates.dat.data[:,1] *=0.5
...
phihat.interpolate(conditional(abs(xx[0])<0.25,conditional(xx[1]<0.01,1,-1),-1))
...
for iStep in range(0,20):
	...
	#### Generating phi_g from phi_n. Step 2 in the Algorithm
